...
Feature: GCS sink - Verification of GCS Sink plugin
  ...
  Scenario: To verify data is getting transferred successfully from BigQuery to GCS with supported datatypes
    Given Open Datafusion Project to configure pipeline
    When Source is BigQuery
    When Sink is GCS
    Then Connect source as "BigQuery" and sink as "GCS" to establish connection
    ...
    Then Enter the BigQuery source mandatory properties
  #  Then Enter input plugin property: "table" with value: "bqTableName"
    Then Click on the Get Schema button
#    Then Change datatype of fields in output schema with : "bqDatatypeChange1"
    Then Validate "BigQuery" plugin properties
#    Then Verify the Output Schema matches the Expected Schema: "bqDataTypeTestFileSchema1"
...
#    Then Enter GCS sink property path suffix "gcsPathSuffix"
    ...
    Then Verify data is transferred to target GCS bucket
    Then Close the pipeline logs
    Then Validate output file generated by GCS sink plugin is equal to expected output file "BQToDBAllDataTypes"
#    Then Validate the values of records transferred to target GCS is equal to the values from source BigQuery table

  @BQ_SOURCE_DATATYPE_TEST @GCS_SINK_TEST
  Scenario: To verify data is getting transferred successfully from BigQuery to GCS using delimiter format
    Given Open Datafusion Project to configure pipeline
    When Source is BigQuery
    When Sink is GCS
    Then Connect source as "BigQuery" and sink as "GCS" to establish connection
