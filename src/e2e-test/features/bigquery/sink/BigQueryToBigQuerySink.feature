...
Feature: BigQuery sink - Verification of BigQuery to BigQuery successful data transfer
  ...
  Scenario:Validate successful records transfer from BigQuery to BigQuery with Advanced operations Upsert with updating the destination table schema and destination table must exist.
    Given Open Datafusion Project to configure pipeline
    When Expand Plugin group in the LHS plugins list: "Source"
    When Select plugin: "BigQuery" from the plugins list as: "Source"
    When Expand Plugin group in the LHS plugins list: "Sink"
  ...
  Scenario: Verify successful record transfer for the Insert operation with partition type Integer and destination table is existing already.
    ...
    Then Click on the Browse button inside plugin properties
    Then Select connection data row with name: "dataset"
    Then Select connection data row with name: "bqTargetTable"
    Then Wait till connection data loading completes with a timeout of 60 seconds
    Then Verify input plugin property: "dataset" contains value: "dataset"
    Then Verify input plugin property: "table" contains value: "bqTargetTable"
  ...
  Scenario: Verify successful transfer from BigQuery source to BigQuery sink plugin with partition type Time (Date/timestamp/datetime).
    Given Open Datafusion Project to configure pipeline
    When Expand Plugin group in the LHS plugins list: "Source"
    When Select plugin: "BigQuery" from the plugins list as: "Source"
    When Expand Plugin group in the LHS plugins list: "Sink"
