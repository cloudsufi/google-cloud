...
public class PubSubActions {
  ...
  public static void selectDataTypeDropdownOption(String dataType, String option) {
    String pluginPropertyDataCyAttribute = PluginPropertyUtils.getPluginPropertyDataCyAttribute(dataType);
    if (pluginPropertyDataCyAttribute == null) {
      pluginPropertyDataCyAttribute = dataType;
    }

    String optionFromPluginPropertiesFile = PluginPropertyUtils.pluginProp(option);
    if (optionFromPluginPropertiesFile == null) {
      ElementHelper.selectDropdownOption(CdfPluginPropertiesLocators.locatePropertyElement(pluginPropertyDataCyAttribute), PubSubLocators.locateDropdownListItem(option));
    } else {
      ElementHelper.selectDropdownOption(CdfPluginPropertiesLocators.locatePropertyElement(pluginPropertyDataCyAttribute), PubSubLocators.locateDropdownListItem(optionFromPluginPropertiesFile));
    }
  }
